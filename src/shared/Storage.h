...
public:
     ...
     */
    ASCENT_INLINE void LoadBlock(Field* fields, T* Allocated, bool reload = false)
    {
    ...
    void Load(const char* IndexName, const char* FormatString)
    {
        ...
        //uint64 count = 0;
        if (QueryResult* result = WorldDatabase.Query("SELECT * FROM %s", IndexName))
        {
        ...
        //Log.Success("Storage", "Loaded %lu entries loaded from table %s.", count, IndexName);
    }
