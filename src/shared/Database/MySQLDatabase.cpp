 ...
 * Copyright (C) 2005-2012 <http://www.AscEnt.org/>
 *
...
bool MySQLDatabase::Initialize(const char* Hostname, unsigned int port, const char* Username, const char* Password, const char* DatabaseName, uint32 ConnectionCount, uint32 /*BufferSize*/)
{
    ...
    for (uint32 i = 0; i < ConnectionCount; i++)
    {
        if (MYSQL* temp = mysql_init(NULL))
        {
            ...
            if (!mysql_real_connect(temp, Hostname, Username, Password, DatabaseName, port, NULL, 0))
            {
...
QueryResult* MySQLDatabase::_StoreQueryResult(DatabaseConnection* con)
{
    ...
    if (!uRows || !uFields || !pRes)
    {
        mysql_free_result(pRes);
        return NULL;
    }

    if (MySQLQueryResult* res = new MySQLQueryResult(pRes, uFields, uRows))
    {
        res->NextRow();
        return res;
    }
...
bool MySQLDatabase::_Reconnect(MySQLDatabaseConnection* conn)
{
    ...
    MYSQL* temp2 = mysql_real_connect(temp, mHostname.c_str(), mUsername.c_str(), mPassword.c_str(), mDatabaseName.c_str(), mPort, NULL, 0);
    if (!temp2)
    {
