...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
	...
	if(_player->HasFlag(UNIT_FIELD_FLAGS,UNIT_FLAG_LOCK_PLAYER))
	{
		_player->SendCastResult(spellId, SPELL_FAILED_SPELL_IN_PROGRESS, cn, 0);
		return;
	}
	...
	//if (tmpItem->GetEntry() == 40732)
	//	itemProto->gossip_script = sScriptMgr.GetDefaultGossipScript();
	//	return;
...
void WorldSession::HandleAddDynamicTargetOpcode(WorldPacket & recvPacket)
{
	...
	else
	{
		...
		else if( _player->m_CurrentCharm )			// trinket?
		{
			...
			//	Spell* pSpell = new Spell(caster, sp, false, 0);
