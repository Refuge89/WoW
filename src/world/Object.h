...
public:
  ...
  //! True if object exists in world
	...
	ASCENT_INLINE bool IsVehicle() { return m_isVehicle; }
	ASCENT_INLINE bool IsGameObject() { return m_objectTypeId == TYPEID_GAMEOBJECT; }
	bool IsPet();

	virtual bool IsTotem() { return false; }

	//! This includes any nested objects we have, inventory for example.
	virtual uint32 ASCENT_FASTCALL BuildCreateUpdateBlockForPlayer( ByteBuffer *data, Player* target );
	uint32 ASCENT_FASTCALL BuildValuesUpdateBlockForPlayer( ByteBuffer *buf, Player* target );
	uint32 ASCENT_FASTCALL BuildValuesUpdateBlockForPlayer( ByteBuffer * buf, UpdateMask * mask );
	uint32 ASCENT_FASTCALL BuildOutOfRangeUpdateBlock( ByteBuffer *buf );
	...
	void ASCENT_FASTCALL ModFloatValue(const uint32 index, const float value );
	...
	void ASCENT_FASTCALL SetUInt32Value( const uint32 index, const uint32 value );

	//! Set uint64 property
	void ASCENT_FASTCALL SetUInt64Value( const uint32 index, const uint64 value );

	//! Set float property
	void ASCENT_FASTCALL SetFloatValue( const uint32 index, const float value );

	void ASCENT_FASTCALL SetFlag( const uint32 index, uint32 newFlag );

	void ASCENT_FASTCALL RemoveFlag( const uint32 index, uint32 oldFlag );

	void ASCENT_FASTCALL SetByteFlag( const uint32 index, const uint32 flag, uint8 newFlag);
	bool ASCENT_FASTCALL HasByteFlag( const uint32 index, const uint32 flag, uint8 checkFlag);
	void ASCENT_FASTCALL RemoveByteFlag(const uint32 index, const uint32 flag, uint8 checkFlag);
	void ASCENT_FASTCALL SetFlag64( const uint32 index, uint64 newFlag );
	void ASCENT_FASTCALL RemoveFlag64( const uint32 index, uint64 newFlag );
	...
	ASCENT_INLINE void AquireInrangeLock(){ m_inrangechangelock.Acquire(); }
	ASCENT_INLINE void ReleaseInrangeLock(){ m_inrangechangelock.Release(); }

	//m_sameFactsInRange
	ASCENT_INLINE void AquireSameFacInRangeLock(){ m_samefactsinrangechangelock.Acquire(); }		
	ASCENT_INLINE void ReleaseSameFacInRangeLock(){ m_samefactsinrangechangelock.Release(); }

	//m_inrangeplayerchangelock
	ASCENT_INLINE void AquirePlayerInRangeLock(){ m_inrangeplayerchangelock.Acquire(); }		
	ASCENT_INLINE void ReleasePlayerInRangeLock(){ m_inrangeplayerchangelock.Release(); }
	...
	void ASCENT_FASTCALL SendMessageToSet(WorldPacket *data, bool self,bool myteam_only=false);
...
public:
	...
	ASCENT_INLINE Loot* GetLoot() { return &m_loot; }
