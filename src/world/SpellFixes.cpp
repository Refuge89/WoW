...
//				}
	...
	/*	if( sp->NameHash == SPELL_HASH_SUMMON_EYE_OF_ACHERUS || sp =  CheckAndReturnSpellEntry(51852))
		{
		...
		// Pure hax
		if( sp->NameHash == SPELL_HASH_SUMMON_EYE_OF_ACHERUS )
		{
			sp->Effect[ 2 ] = SPELL_EFFECT_DUMMY; //Spawn GO
			switch(sp->Id)
			{
...
*/
	...
	//Eye of Acherus Recall Spells
/*	int RecallEyeSpells[2] = {51888, 51852};
	for(i = 0; i < 2; i++)
	{
		sp = CheckAndReturnSpellEntry(RecallEyeSpells[i]);
		if(sp != NULL)
		{
		sp->Effect[ 2 ] = SPELL_EFFECT_DUMMY;
					sp->Effect[ 2 ] = SPELL_EFFECT_DUMMY; //Spawn GO
