...
#ifdef WIN32
	_player->BroadcastMessage("Server: %sSunriseEmu %s r%u/%s-Win-%s %s", MSG_COLOR_WHITE, BUILD_TAG,
		BUILD_REVISION, CONFIG, ARCH, MSG_COLOR_LIGHTBLUE);
#else
	_player->BroadcastMessage("Server: %sSunriseEmu %s r%u/%s-%s %s", MSG_COLOR_WHITE, BUILD_TAG,
...
void WorldSession::HandleCharacterCustomization( WorldPacket & recv_data )
{
	...
	if(!sWorld.VerifyName(name.c_str(), name.length()))
	{
		uint8 err = E_CHAR_NAME_INVALID_CHARACTER;
	...
	if(g_characterNameFilter->Parse(name, false))
	{
		uint8 err = E_CHAR_NAME_RESERVED;
		OutPacket(SMSG_CHAR_CUSTOMIZE, 1, &err);
		return;
	}
	...
	{
		uint8 err = E_CHAR_CREATE_NAME_IN_USE;
		OutPacket(SMSG_CHAR_CUSTOMIZE, 1, &err);
		return;
	}
