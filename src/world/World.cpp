...
void World::Rehash(bool load)
{
	...
	{
		#ifdef WIN32
		Config.MainConfig.SetSource("world.conf", true);
		#else
	...
	// Max Gold Settings
	GoldCapEnabled = Config.MainConfig.GetBoolDefault("GoldSettings", "EnableGoldCap", true);
	GoldLimit = Config.MainConfig.GetIntDefault("GoldSettings", "MaximumGold", 214000);
	if(GoldLimit)
