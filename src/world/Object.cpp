	...
	if(updateMask->GetBit(OBJECT_FIELD_GUID) && target)	   // We're creating.
	{
		...
		if(GetTypeId() == TYPEID_UNIT && pThis->Tagged)
		{
			...
			if(pThis->TaggerGuid == target->GetGUID())
			{
				...
				if( pThis->loot.gold || pThis->loot.items.size() )
									...
									if(pTagger->InGroup())
									{
										...
										for(uint32 i = 0; i < pGroup->GetSubGroupCount(); i++) {
											for(itr = pGroup->GetSubGroup(i)->GetGroupMembersBegin(); itr != pGroup->GetSubGroup(i)->GetGroupMembersEnd(); ++itr)
											{
												...
												{
													if(active_player_count>MAX_GROUP_SIZE_RAID)
														active_player_count = MAX_GROUP_SIZE_RAID;
													else if(active_player_count<0)
