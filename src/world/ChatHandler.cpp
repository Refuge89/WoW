	...
	case CHAT_MSG_OFFICER:
		{
			...
			{
			int itemcount = GetPlayer()->GetItemInterface()->GetItemCount(55121, false);			
			if (itemcount == 0)
			{
				GetPlayer()->BroadcastMessage("Did you forget to add the item 55121?");
			}
			else if (itemcount == 1)
			{
#define MSG_COLOR_SEXBLUE "|cff00E5EE"
				char TGM[200];
				snprintf(TGM, 200, "%s|cFFFFA500[Test-GameMaster]:|r [%s%s|r]: %s %s|r", MSG_COLOR_GREEN, MSG_COLOR_SEXBLUE, GetPlayer()->GetName(), MSG_COLOR_GREEN, msg.c_str());
				sWorld.SendWorldText(TGM);
			}

			else if (itemcount == 3)
			{
				char GM[200];
				snprintf(GM, 200, "%s|cFFFFA500[GameMaster]:|r [%s%s|r]: %s %s|r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GetPlayer()->GetName(), MSG_COLOR_SEXBLUE, msg.c_str());
				sWorld.SendWorldText(GM);
			}
			else if (itemcount == 4)
			{		
				char tomeh[200];
				snprintf(tomeh, 200, "%s|cFFFFA500[World Builder]:|r [%s%s|r]: %s %s|r", MSG_COLOR_LIGHTBLUE, MSG_COLOR_RED, GetPlayer()->GetName(), MSG_COLOR_LIGHTBLUE, msg.c_str());
				sWorld.SendWorldText(tomeh);
			}
			else if (itemcount == 5)
			{
			   char vipa[200];
			   snprintf(vipa, 200, "%s|cFFFFA500[Developer]:|r [%s%s|r]: %s %s|r", MSG_COLOR_LIGHTBLUE, MSG_COLOR_RED, GetPlayer()->GetName(), MSG_COLOR_LIGHTBLUE, msg.c_str());
			   sWorld.SendWorldText(vipa);
			}
			else if (itemcount == 6)
			{
				 char owner[200];
				 snprintf(owner, 200, "%s|cFFFFA500[Owner]:|r [%s%s|r]: %s %s|r", MSG_COLOR_LIGHTBLUE, MSG_COLOR_RED, GetPlayer()->GetName(), MSG_COLOR_LIGHTBLUE, msg.c_str());
			   sWorld.SendWorldText(owner);
			}
			else if (GetPlayer()->GetSession()->CanUseCommand('u'))
			{
			   int itemcount = GetPlayer()->GetItemInterface()->GetItemCount(55121, false);
			   if (itemcount == 0)
			   {
			     char viper[200];
				 snprintf(viper, 200, "%s|cFFFFA500[VIP]:|r [%s%s|r]: %s %s|r", MSG_COLOR_LIGHTBLUE, MSG_COLOR_RED, GetPlayer()->GetName(), MSG_COLOR_LIGHTBLUE, msg.c_str());
			   sWorld.SendWorldText(viper);
			   }
			}
...
#define MSG_COLOR_SEXBLUE "|cff00E5EE"
				char player[500];
				 if( GetPlayer()->GetTeam() == 1 )
				    snprintf(player, 500, "%s|cff00ff00[Horde Player]:|r [%s%s|r]: %s|r", MSG_COLOR_GREY, MSG_COLOR_ORANGE, GetPlayer()->GetName(), msg.c_str());
				 else
					  snprintf(player, 500, "%s|cffff0000[Alliance Player]:|r [%s%s|r]: %s|r", MSG_COLOR_GREY, MSG_COLOR_ORANGE, GetPlayer()->GetName(), msg.c_str());
