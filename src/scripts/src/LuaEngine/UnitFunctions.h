...
class LuaUnit
{
	...
	static int GossipMenuAddItem(lua_State * L, Unit * ptr)
	{
		...
		Menu->AddItem(icon, menu_text, IntId, extra, boxmoney,boxmessage );
		return 0;
	}
...
*/
		...
		{
			if(!ptr) return 0;
			lua_pushnumber(L, ptr->m_phaseMode);
			return 1;
		}
		...
		static int SpawnCreature(lua_State* L, Unit* ptr)
		{
			...
			uint32 phase = luaL_optint(L, 11, ptr->m_phaseMode);
			...
			pCreature->SetWeaponDisplayId(MELEE, equip1);
			pCreature->SetWeaponDisplayId(OFFHAND, equip2);
			pCreature->SetWeaponDisplayId(RANGED, equip3);
		...
		static int SpawnGameObject(lua_State* L, Unit* ptr)
		{
			...
			//		sEventMgr.AddEvent(go, &GameObject::ExpireAndDelete, EVENT_GAMEOBJECT_UPDATE, duration, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
					sEventMgr.ModifyEventTimeAndTimeLeft(go, EVENT_GAMEOBJECT_EXPIRE, duration);
				if(save)
					go->SaveToDB();
				PUSH_GO(L, go);
			}
...
/*		static int GetNativeFaction(lua_State* L, Unit* ptr)
		{
			...
			if(ptr->IsPlayer())
			{
				RET_INT(TO_PLAYER(ptr)->GetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE, 1);
			}
			else
			{
		...
		static int SetModel(lua_State* L, Unit* ptr)
		{
			...
			if(ptr != NULL)
				ptr->SetUInt32Value(UNIT_FIELD_DISPLAYID, model);
			else
				RET_BOOL(false)
				RET_BOOL(true)
			}
		...
		static int CreateWaypoint(lua_State* L, Unit* ptr)
		{
			...
			if(pCreature->GetAIInterface()->addWayPoint(wp))
		...
		static int AddItem(lua_State* L, Unit* ptr)
		{
			...
			if(add == NULL)
			{
				...
				if(plr->GetItemInterface()->AddItemToFreeSlot(add))
					plr->GetSession()->SendItemPushResult(add,false, true, false, true, plr->GetItemInterface()->LastSearchItemBagSlot(),
					                        plr->GetItemInterface()->LastSearchItemSlot(), count);
				else
					delete add;
			}
			else
			{
				...
				plr->GetSession()->SendItemPushResult(add,false, true, false, false,
		...
		static int GetInventoryItemById(lua_State* L, Unit* ptr)
		{
			...
			if(slot == -1)  //check bags
			{
				for(uint8 contslot = INVENTORY_SLOT_BAG_START; contslot != INVENTORY_SLOT_BAG_END; contslot++)
				{
					...
					if(bag == NULL)
						continue;
					for(uint8 bslot = 0; bslot != bag->GetCount(); bslot++)
					{
					...
					else
					{
						...
						else
						{
							...
							if(qst->srcitem && qst->srcitem != qst->receive_items[0])
							{
								...
								if(item)
								{
									item->SetCount(qst->srcitemcount ? qst->srcitemcount : 1);
									if(!plr->GetItemInterface()->AddItemToFreeSlot(item))
										item->Destructor();
								}
							}
		...
		static int SetHealth(lua_State* L, Unit* ptr)
		{
			...
			if(ptr != NULL && val > 0)
			{
				if((uint32)val > ptr->GetUInt32Value(UNIT_FIELD_MAXHEALTH))
					ptr->SetHealthPct(ptr->GetUInt32Value(UNIT_FIELD_MAXHEALTH));
				else
					ptr->SetHealthPct(val);
			}
			return 0;
...
/*		static int ClearHateList(lua_State* L, Unit* ptr)
		...
		static int SetMaxMana(lua_State* L, Unit* ptr)
		{
			...
			if(ptr != NULL && val > 0)
			{
				if((uint32)val < ptr->GetUInt32Value( UNIT_FIELD_POWER1 + POWER_TYPE_MANA))
					ptr->SetPower(POWER_TYPE_MANA, val);
				ptr->SetUInt32Value( UNIT_FIELD_MAXPOWER1 + POWER_TYPE_MANA, val);
			}
			return 1;
		}
		...
		static int IsInPhase(lua_State* L, Unit* ptr)
		{
			...
			lua_pushboolean(L, ((ptr->m_phaseMode & phase) != 0) ? 1 : 0);
			return 1;
		}
	...
	/*	static int SendAIReaction(lua_State* L, Unit* ptr)
		...
		static int GetManaPct(lua_State* L, Unit* ptr)
		{
			...
			if(ptr->GetPowerType() == (uint8)POWER_TYPE_MANA)
				lua_pushnumber(L, (int)(ptr->GetUInt32Value( UNIT_FIELD_POWER1 + POWER_TYPE_MANA) * 100.0f / ptr->GetUInt32Value( UNIT_FIELD_MAXPOWER1 + POWER_TYPE_MANA)));
			else
				lua_pushnil(L);
			return 1;
		}
		...
		static int GetPowerPct(lua_State* L, Unit* ptr)
		{
			...
			lua_pushnumber(L, (int)(ptr->GetUInt32Value( UNIT_FIELD_POWER1 + powertype) * 100.0f / ptr->GetUInt32Value( UNIT_FIELD_MAXPOWER1 + powertype)));
			return 1;
		}
		...
		static int GetMana(lua_State* L, Unit* ptr)
		{
			if(ptr == NULL)
				...
				lua_pushinteger(L, ptr->GetUInt32Value( UNIT_FIELD_POWER1 + POWER_TYPE_MANA));
		...
		static int GetPower(lua_State* L, Unit* ptr)
		{
			...
			if(powertype == -1)
				powertype = ptr->GetPowerType();
			lua_pushnumber(L, ptr->GetUInt32Value( UNIT_FIELD_POWER1 + powertype));
			return 1;
		}
		...
		static int GetMaxMana(lua_State* L, Unit* ptr)
		{
			if(ptr == NULL)
				...
				lua_pushinteger(L, ptr->GetUInt32Value( UNIT_FIELD_MAXPOWER1 + POWER_TYPE_MANA));
		...
		static int GetMaxPower(lua_State* L, Unit* ptr)
		{
			...
			if(powertype == -1)
				powertype = ptr->GetPowerType();
			lua_pushnumber(L, ptr->GetUInt32Value( UNIT_FIELD_MAXPOWER1 + powertype));
			return 1;
		}
		...
		static int SetMaxPower(lua_State* L, Unit* ptr)
		{
			...
			if(powertype == -1)
				powertype = ptr->GetPowerType();
			ptr->SetUInt32Value( UNIT_FIELD_MAXPOWER1 + powertype, amount);
			return 0;
		}
		...
		static int SetPowerPct(lua_State* L, Unit* ptr)
		{
			...
			if(powertype == -1)
				powertype = ptr->GetPowerType();
			ptr->SetPower(powertype, (int)(amount / 100) * (ptr->GetUInt32Value( UNIT_FIELD_MAXPOWER1 + powertype)));
			return 0;
		}
...
/*		static int CreateGuardian(lua_State* L, Unit*  ptr)
		...
		static int GetAITargets(lua_State* L, Unit* ptr)
		{
			...
			{
				ret = ptr->GetMapMgr()->GetUnit(itr->first->GetGUID());
				count++;
...
/*		static int AddGroupMember(lua_State* L, Unit* ptr)
		...
		static int GetInRangeGameObjects(lua_State* L, Unit* ptr)
		{
			...
			lua_newtable(L);
			uint32 count = 0;
			for(std::unordered_set<Object*>::iterator itr = ptr->GetInRangeSetBegin(); itr != ptr->GetInRangeSetEnd(); itr++)
			{
				if((*itr)->IsGameObject())
				{
					count++,
		...
		static int SetPlayerSpeed(lua_State* L, Unit* ptr)
		{
			...
			if(Speed < 1 || Speed > 255)
				return 0;
			plr->m_runSpeed = Speed;
			plr->m_swimSpeed = Speed;
			plr->m_walkSpeed = Speed / 2;
			plr->m_flySpeed = Speed * 2;
			return 0;
		}
		...
		static int SetLevel(lua_State* L, Unit* ptr)
		{
			...
			if(level <= sWorld.LevelCap && level > 0)
			{
				if(ptr->IsPlayer())
				{
					...
					ptr->SetUInt32Value(UNIT_FIELD_LEVEL, level);
			}
			return 0;
		}
		...
		static int InterruptCurrentSpell(lua_State* L, Unit* ptr)
		{
			if(!ptr) return 0;
		...
		static int DealGoldCost(lua_State* L, Unit* ptr)
		{
			...
			if(debt < 0) return 0;
			if(!plr->GetUInt32Value((PLAYER_FIELD_COINAGE) >= (debt)))
				lua_pushboolean(L, 0);
			else
			{
		...
		static int GetTarget(lua_State* L, Unit* ptr)
		{
			...
			TEST_PLAYER()
			Player* plr = TO_PLAYER(ptr);
			Unit* target = plr->GetMapMgr()->GetUnit(plr->GetUInt64Value( UNIT_FIELD_TARGET ));
			if(target != NULL)
				PUSH_UNIT(L, target);
			else
				lua_pushnil(L);
...
/*		static int GetSelectedGO(lua_State* L, Unit* ptr)
		{
			TEST_PLAYER()
			Player* plr = TO_PLAYER(ptr);
			PUSH_GO(L, plr->GetSelection()->GetGUID());
			return 1;
		}*/
		...
		static int HasTitle(lua_State* L, Unit* ptr)
		{
			...
			int title = luaL_checkint(L, 1);
			Player* plr = TO_PLAYER(ptr);
			if(plr->GetUInt64Value( PLAYER__FIELD_KNOWN_TITLES +(RankTitles(title))))
				lua_pushboolean(L, 1);
			else
				lua_pushboolean(L, 0);
			return 1;
			...
			if(guidtype == HIGHGUID_TYPE_UNIT)
			{
				...
				switch(loot_type)
				{
					default:
						...
						break;
					case 2:
						lootmgr.FillGatheringLoot(&pUnit->loot, pUnit->GetEntry());
						loot_type2 = 2;
						break;
					case 3:
			...
			else if(guidtype == HIGHGUID_TYPE_GAMEOBJECT)
			{
				...
				switch(loot_type)
				{
					default:
						...
						break;
					case 5:
						lootmgr.FillGatheringLoot(&pGO->loot, pGO->GetEntry());
						loot_type2 = 2;
						break;
				}
			}
		...
		static int AddLoot(lua_State* L, Unit* ptr)
		{
			...
			lootmgr.AddLoot(&ptr->loot, itemid, mincount, maxcount , 0);
			return 0;
		}
		...
		static int VendorAddItem(lua_State* L, Unit* ptr)
		{
			...
			if(itemid && amount)
				ctr->AddVendorItem(itemid, amount );
			return 0;
		}
...
/*		static int IsGroupedWith(lua_State* L, Unit* ptr)
	...
	/*	static int SendGuildInvite(lua_State* L, Unit* ptr)
		{
			...
			else if(plyr->GetTeam() != sender->GetTeam() && sender->GetSession()->GetPermissionCount() == 0 )
			{
	...
	/*	static int UnbanFromChannel(lua_State* L, Unit* ptr)
		...
		static int SetMovementFlags(lua_State* L, Unit* ptr)
		{
			...
			{
				ptr->GetAIInterface()->m_moveSprint = true;
			}
			return 0;
		}
		...
		//	ptr == TEST_PLAYER();
		//	TO_PLAYER(ptr)->m_specs[spec].talents( points );
...
/*		static int GetTalentPoints(lua_State* L, Unit* ptr)
		...
		//		ptr->SendChatMessage(typ, lang, message, delay);
				sEventMgr.AddEvent(TO_UNIT(ptr), &Unit::SendChatMessage, typ, lang, message, EVENT_UNIT_CHAT_MSG , delay, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
			return 0;
		}
	...
	/*	static int IsRooted(lua_State* L, Unit* ptr)
		...
		{
			TEST_UNITPLAYER_RET();
			for(uint32 x = 0; x < MAX_POSITIVE_AURAS+16; ++x)
			{
...
/*		static int SaveToInstance(lua_State* L, Unit* ptr)
	...
	/*	static int GetQuestLogSlot(lua_State* L, Unit* ptr)
		...
		static int IsOnVehicle( lua_State *L, Unit *ptr ){
			...
			if( ( ptr->m_CurrentVehicle != NULL ) || ( ptr->IsPlayer() && ptr->IsVehicle() ) )
				lua_pushboolean( L, 1 );
			else
				lua_pushboolean( L, 0 );
		...
		static int SpawnAndEnterVehicle( lua_State *L, Unit *ptr ){
			...
			if( ( cp->vehicle_entry == 0 ) && ( p == NULL ) && ( !p->GetSession()->HasGMPermissions() ) )
				return 0;
		...
		static int DismissVehicle( lua_State *L, Unit *ptr ){
			...
			if( ptr->m_CurrentVehicle != NULL )
				v = ptr->m_CurrentVehicle;
			if( v == NULL )
				return 0;

			v->RemovePassenger(ptr);
			
			Unit *o = v->GetSummonOwner();

			if( o->IsPlayer() )
				o->RemoveAllAuras( SPELL_AURA_MOUNTED );
			else
				o->Delete();
		...
		static int AddVehiclePassenger( lua_State *L, Unit *ptr ){
			...
			if( ptr->IsPlayer() && ( ptr->m_CurrentVehicle != NULL ) )
			//	v = ptr->GetVehicleComponent();
				((Vehicle*)ptr)->AddPassenger(ptr);
