...
*/
		...
		static int SpawnGameObject(lua_State* L, GameObject* ptr)
		{
			...
			if(duration)
			//	sEventMgr.AddEvent(go, &GameObject::ExpireAndDelete,1, EVENT_GAMEOBJECT_UPDATE, duration, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		...
		static int ChangeScale(lua_State* L, GameObject* ptr)
		{
			...
			ptr->SetFloatValue(OBJECT_FIELD_SCALE_X, nScale);
			if(updateNow)
			{
				MapMgr* mapMgr = ptr->GetMapMgr();
		...
		static int SetScale(lua_State* L, GameObject* ptr)
		{
			...
			if(scale > 0)
				ptr->SetFloatValue(OBJECT_FIELD_SCALE_X, scale);
			return 0;
		}
/*		static int GetScale(lua_State* L, GameObject* ptr)
		{
			TEST_GO();
			lua_pushnumber(L, static_cast<Object*>(ptr)->GetScale());
			return 1;
		}*/
		...
		static int Damage( lua_State *L, GameObject *ptr ){
			...
			ptr->DealDamage(TO_UNIT(guid), damage,  guid, spellid, true );
