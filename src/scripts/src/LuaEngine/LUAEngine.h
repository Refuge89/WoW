...
class LuaEngine
{
	...
	public:
		...
		ASCENT_INLINE void PUSH_BOOL(bool bewl)
		{
			if(bewl)
		...
		ASCENT_INLINE void PUSH_NIL(lua_State* L = NULL)
		{
			if(L == NULL)
				lua_pushnil(lu);
			else
		...
		ASCENT_INLINE void PUSH_INT(int32 value)
		{
			lua_pushinteger(lu, value);
		}
		ASCENT_INLINE void PUSH_UINT(uint32 value)
		{
			lua_pushnumber(lu, value);
		}
		ASCENT_INLINE void PUSH_FLOAT(float value)
		{
			lua_pushnumber(lu, value);
		}
		ASCENT_INLINE void PUSH_STRING(const char* str)
		{
			lua_pushstring(lu, str);
		}
		void RegisterCoreFunctions();

		ASCENT_INLINE Mutex & getLock() { return call_lock; }
		ASCENT_INLINE Mutex & getcoLock() { return co_lock; }
		ASCENT_INLINE lua_State* getluState() { return lu; }
		...
		ASCENT_INLINE std::multimap<uint32, LuaCreature*> & getLuCreatureMap() { return m_cAIScripts; }
		ASCENT_INLINE std::multimap<uint32, LuaGameObjectScript*> & getLuGameObjectMap() { return m_gAIScripts; }
		ASCENT_INLINE HM_NAMESPACE::hash_map<uint32, LuaQuest*> & getLuQuestMap() { return m_qAIScripts; }
		ASCENT_INLINE HM_NAMESPACE::hash_map<uint32, LuaInstance*> & getLuInstanceMap() { return m_iAIScripts; }
		ASCENT_INLINE HM_NAMESPACE::hash_map<uint32, LuaGossip*> & getUnitGossipInterfaceMap() { return m_unitgAIScripts; }
		ASCENT_INLINE HM_NAMESPACE::hash_map<uint32, LuaGossip*> & getItemGossipInterfaceMap() { return m_itemgAIScripts; }
		ASCENT_INLINE HM_NAMESPACE::hash_map<uint32, LuaGossip*> & getGameObjectGossipInterfaceMap() { return m_gogAIScripts; }
		ASCENT_INLINE std::set<int> & getThreadRefs() { return m_pendingThreads; }
		ASCENT_INLINE std::set<int> & getFunctionRefs() { return m_functionRefs; }
		ASCENT_INLINE std::map< uint64, std::set<int> > & getObjectFunctionRefs() { return m_objectFunctionRefs; }
	...
	protected:
		...
		class ArcLuna
		{
			...
			private:
				...
				ASCENT_INLINE static void tostring(char* buff, void* obj)
				{
					sprintf(buff, "%p", obj);
				}
