...
{
	sLog.outString("exp_engine_unload was called");
}
...
{
	GET_LOCK
	sLuaMgr.BeginCall(m_luaDummySpells[pSpell->GetSpellProto()->Id]);
...
class LuaCreature : public CreatureAIScript
{
	public:
		...
		~LuaCreature()
		{}
		ASCENT_INLINE void SetUnit(Creature* ncrc) { _unit = ncrc; }
...
class LuaGameObjectScript : public GameObjectAIScript
{
	public:
		...
		~LuaGameObjectScript() {}
		ASCENT_INLINE GameObject* getGO() { return _gameobject; }
		void OnCreate()
		{
			CHECK_BINDING_ACQUIRELOCK
