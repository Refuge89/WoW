...
{ 
	SetDisplayWeaponIds(pMainHand ? _unit->GetProtoItemDisplayId(MELEE) : 0, pOffHand ? _unit->GetProtoItemDisplayId(OFFHAND) : 0);
}
...
{
	//Main Hand
	_unit->SetWeaponDisplayId(MELEE,pItem1Id);
	
	//Off Hand
	_unit->SetWeaponDisplayId(OFFHAND,pItem2Id);
}
...
{
	for( std::unordered_set< Player* >::iterator PlayerIter = _unit->GetInRangePlayerSetBegin(); PlayerIter != _unit->GetInRangePlayerSetEnd(); ++PlayerIter ) 
	{// need testing
...
void EventFunc_ChangeGoState(MoonScriptCreatureAI* pCreatureAI, int32 pMiscVal)
{
	...
	for ( GameObjectMap::iterator GOIter = pInstance->m_gameObjectStorage.begin(); GOIter != pInstance->m_gameObjectStorage.end(); ++GOIter )
	{
		pSelectedGO = (*GOIter).second;
