...
class AcherusSoulPrison : GameObjectAIScript
{
	...
	/*	AcherusSoulPrison(GameObject * goinstance) : GameObjectAIScript(goinstance) {}
		...
		{
			QuestLogEntry * qle = pPlayer->GetQuestLogForEntry( 12848 );
			if( !qle || qle->GetMobCount(0) != 0 )
				return; 
...
class TheEyesOfAcherus : public MoonScriptCreatureAI
{
	...
	TheEyesOfAcherus(Creature* pCreature) : MoonScriptCreatureAI(pCreature) 
	{
		...
		//	RegisterAIUpdateEvent(500);
			...
			_unit->SetUInt32Value(UNIT_FIELD_DISPLAYID, _unit->GetCreatureInfo()->Female_DisplayID);
		}
};
...
bool EyeOfAcherus(uint32 i, Spell * pSpell)
{
	...
	CreatureProto * proto = CreatureProtoStorage.LookupEntry(EyeOfAcheruss);
	if ( !proto )
		return true;
...
/*	p_caster->RemoveFieldSummon();
	...
	s->GetScript()->RegisterAIUpdateEvent(1000);*/
	//s->CreateWayPoint(1,609,1822.982056f, -5927.867676f, 154.357071f, 4.487767f);// working
...
class Massakre : public CreatureAIScript
{
	public:
	ADD_CREATURE_FACTORY_FUNCTION(Massakre );
	Massakre(Creature* pCreature) : CreatureAIScript(pCreature)
	{
	};
...
//	uint32 crusade_Massakre[]= { 28834, 28849, 28856, 28850, 0 };
//	uint32 crusade_Massakre[]= { 28834, 28850, 28856};
	mgr->register_creature_script(28834, &Massakre::Create);
//	mgr->register_creature_script(28850, &Massakre::Create);
	mgr->register_creature_script(28856, &Massakre::Create);
...
//	uint32 special_surprise[] = { 29032, 29061, 29065, 29067, 29068, 29070, 29074, 29072, 29073, 29071, 0 };
	...
	mgr->register_creature_script(EyeOfAcheruss, &TheEyesOfAcherus::Create); //Skyguard Windcharger
}